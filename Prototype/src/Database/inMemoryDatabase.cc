...
void InMemoryDatabase::DeleteDeltaIndex(const std::string &key)
{
    if (delta_index.find(key) != delta_index.end())
    {
        tool::Logging("InMemoryDatabase", "delete old delta\n");
        delta_index.erase(key);
    }
    else
    {
