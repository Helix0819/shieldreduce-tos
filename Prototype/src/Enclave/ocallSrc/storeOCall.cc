...
// Keep in sync with _test_buffer allocation in src/Util/clientVar.cc
// _test_buffer is allocated to a large size in clientVar.cc (e.g., 200000 * CHUNK_HASH_SIZE)
#ifndef TEST_BUFFER_BYTES
#define TEST_BUFFER_BYTES (200000 * CHUNK_HASH_SIZE)
#endif
...
{
    // tool::Logging("DEBUG", "getRefContainer\n");
    ClientVar *outClientPtr = (ClientVar *)outClient;
    OutQuery_t *outQuery = &outClientPtr->_outQuery;
    OutQueryEntry_t *entry = outQuery->outQueryBase;
    string tmpbaseChunkAddress;
    ...
    if (entry->deltaFlag == OUT_DELTA)
    {
        ...
        {
            if (entry->basechunkAddr.containerName[i] != 0)
            {
                allZero = false;
                break;
            }
        }
        // tool::Logging("DEBUG", "容器ID是否全为0: %s\n", allZero ? "是" : "否");
...
#endif
    // tool::Logging(myName_.c_str(), " update delta index done\n");
    return;
}

void Ocall_UpdateDeltaIndexOnly(void *outClient, size_t chunkNum)
{
    ClientVar *outClientPtr = (ClientVar *)outClient;
    uint8_t *tmpBuffer = outClientPtr->_process_buffer;
#if (MULTI_CLIENT == 1)
    ...
    oldBaseChunkHash.assign((char *)tmpBuffer + offset, CHUNK_HASH_SIZE);
    offset += CHUNK_HASH_SIZE;
    indexStoreObj_->DeleteDeltaIndex(oldBaseChunkHash);
    string baseChunkHash;
    string deltaChunkHash;
    for (int i = 0; i < chunkNum; i++)
    {
    ...
    // tool::Logging("OCALL", "UpdateDeltaIndexOnly: chunkNum=%zu, delta_index.size=%zu\n", chunkNum, indexStoreObj_->delta_index.size());
    // tool::Logging("DEBUG", "finish insert delta index, delta index size is : %lu\n", indexStoreObj_->delta_index.size());
...
void Ocall_Localrevise(void *outClient)
{
    ...
    for (unordered_map<string, string>::iterator it = indexStoreObj_->_local_tmp_map.begin(); it != indexStoreObj_->_local_tmp_map.end(); it++)
    {
        tmppair = {it->first, it->second};
        local_greedy_basemap[it->first] = it->second;
        // TODO insert into it->first delta index
        auto it2 = indexStoreObj_->skipMap.find(it->first);
        if (it2 != indexStoreObj_->skipMap.end())
        {
            for (size_t i = 0; i < indexStoreObj_->skipMap[it->first].size(); i++)
            {
                ...
                // tmppair2 = {indexStoreObj_->skipMap[it->first][i], it->second};
                // local_greedy_basemap[indexStoreObj_->skipMap[it->first][i]] = it->second;
                indexStoreObj_->InsertDeltaIndex(it->first, indexStoreObj_->skipMap[it->first][i]);
            }
        }
    }
    // fprintf(stderr, "map_size %d\n",outClientPtr->_local_map.size());
...
void Ocall_GetAllDeltaIndex(void *outClient)
{
    ClientVar *outClientPtr = (ClientVar *)outClient;
    uint8_t *buffer = outClientPtr->_test_buffer;
    size_t offset = 0;
    ...
    if (!iter_initialized)
    {
        delta_iter = indexStoreObj_->extensionMap.begin();
        iter_initialized = true;
    }

    // 若无数据，返回 0 并重置迭代器
    if (delta_iter == indexStoreObj_->extensionMap.end())
    {
        uint32_t zero = 0;
    ...
    // 预留数量位置，稍后回填
    memcpy(buffer, &written, sizeof(uint32_t));
    offset += sizeof(uint32_t);

    // 在不超过缓冲区容量的前提下序列化本批次（仅整组写入；若当前组放不下则结束本批）
#if (MULTI_CLIENT == 1)
    pthread_rwlock_rdlock(&outLogLck_);
#endif
    while (delta_iter != indexStoreObj_->extensionMap.end())
    {
        const string &baseChunkFP = delta_iter->first;
        const vector<string> &deltaChunks = delta_iter->second;

        // 当前组完整占用空间：baseFP + count + delta 列表
        size_t need = CHUNK_HASH_SIZE + sizeof(uint32_t) + (size_t)deltaChunks.size() * CHUNK_HASH_SIZE;
        // 若本组放不下当前批剩余空间，则结束本批，留待下批写入
        if (offset + need > TEST_BUFFER_BYTES)
            break;

        // 写入整组
        memcpy(buffer + offset, baseChunkFP.data(), CHUNK_HASH_SIZE);
        offset += CHUNK_HASH_SIZE;
        uint32_t deltaCount = static_cast<uint32_t>(deltaChunks.size());
        memcpy(buffer + offset, &deltaCount, sizeof(uint32_t));
        offset += sizeof(uint32_t);
        for (const string &deltaFP : deltaChunks)
        {
            memcpy(buffer + offset, deltaFP.data(), CHUNK_HASH_SIZE);
            offset += CHUNK_HASH_SIZE;
        }
        ++written;
        ++delta_iter;

        if (written >= DELTA_BATCH_SIZE)
            break;
    }
#if (MULTI_CLIENT == 1)
    pthread_rwlock_unlock(&outLogLck_);
...
void Ocall_OneRecipe(void *outClient)
{
    ...
    if (queryresult)
    {
        // tool::Logging("OCALL", "Key %02X OneRecipe: QueryBuffer found\n");
        memcpy(&entry->chunkAddr, &tmpChunkAddress[0], sizeof(RecipeEntry_t));
    }
    else
    {
        tool::Logging("OCALL", "OneRecipe: QueryBuffer not found\n");
...
void Ocall_OneContainer(void *outClient)
{
    ...
    // tool::Logging("DEBUG", "最终文件路径: %s and size is %d\n", readFileNameStr.c_str(), readFileNameStr.size());
    containerIn.open(readFileNameStr, ifstream::in | ifstream::binary);
    if (!containerIn.is_open())
    {
        // fprintf(stderr, "there is something wrong\n");
        // fprintf(stderr, "Base chunk container: %s\n", readFileNameStr.c_str());
        tool::Logging("OCALL", "OneContainer: open failed: %s\n", readFileNameStr.c_str());
        entry->offlineFlag = false;
        return;
    }
    // tool::Logging("DEBUG", "1111: \n");
    containerIn.seekg(0, ios_base::end);
    // tool::Logging("DEBUG", "22 \n");
    int readSize = containerIn.tellg();
    // tool::Logging("DEBUG", "33 \n");
    containerIn.seekg(0, ios_base::beg);
    // tool::Logging("DEBUG", "44 \n");
    int containerSize = 0;
    containerSize = readSize;
    containerIn.read((char *)tmpOcallcontainer, containerSize);
    if (containerIn.gcount() != containerSize)
    ...
    // tool::Logging("DEBUG", "55 \n");
    containerIn.close();
    //  tool::Logging("DEBUG", "66 \n");
    entry->containerbuffer = tmpOcallcontainer;
    entry->offlineFlag = true;
    // tool::Logging("OCALL", "OneContainer: loaded %s, size=%d\n", readFileNameStr.c_str(), containerSize);
    //  tool::Logging("DEBUG", "77 \n");
    return;
}
...
void Ocall_SavehotBaseContainer(const char *containerID, uint8_t *containerBody, size_t currentSize)
{
    FILE *containerFile = NULL;
    string containerNamePrefix_ = "Base-Containers/";
    string containerNameTail_ = config.GetContainerSuffix();
    string fileName(containerID, CONTAINER_ID_LENGTH);
    string fileFullName = containerNamePrefix_ + fileName + containerNameTail_;
    // tool::Logging("DEBUG", "Base container fullFileName is  %s\n", fileFullName.c_str());
    containerFile = fopen(fileFullName.c_str(), "wb");
    if (!containerFile)
    {
        exit(EXIT_FAILURE);
    ...
    fwrite((char *)containerBody, currentSize, 1, containerFile);
    fclose(containerFile);
    // tool::Logging("OCALL", "SavehotBaseContainer: wrote %s, size=%zu\n", fileFullName.c_str(), currentSize);
    return;
}
...
void Ocall_OneDeltaContainer(void *outClient)
{
    ...
    if (!containerIn.is_open())
    {
        ...
        tool::Logging("OCALL", "OneDeltaContainer: open failed: %s\n", readFileNameStr.c_str());
        return;
    }
    ...
    containerIn.close();
    entry->containerbuffer = tmpOcallcontainer;
    // tool::Logging("DEBUG", "get delta container succeed\n", containerSize);
    // tool::Logging("OCALL", "OneDeltaContainer: loaded %s, size=%d\n", readFileNameStr.c_str(), containerSize);
    return;
}
...
{
    tool::Logging("OCALL", "CleanLocalIndex: before clear, local_map.size=%zu, extensionMap.size=%zu\n", indexStoreObj_->_local_map.size(), indexStoreObj_->extensionMap.size());
    indexStoreObj_->_local_map.clear();
    indexStoreObj_->extensionMap.clear();
    tool::Logging("OCALL", "CleanLocalIndex: after clear\n");
}
