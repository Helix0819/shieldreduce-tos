...
int main(int argc, char *argv[])
{
    ...
    // if (needRecoverOffline == true)
    // {
    //     serverThreadObj->RecoverOffline();
    // }
    ...
    while (true)
    {
        tool::Logging(myName.c_str(), "waiting for a new request from a client.\n");

        // 调用修改后的 ListenSSL
        pair<int, SSL *> connection = dataSecurityChannelObj->ListenSSL();
        ...
        {
            // 成功获取到一个有效连接，才去处理它
            tool::Logging(myName.c_str(), "Connection accepted. Starting processing...\n");
            serverThreadObj->Run(clientSSL);
            tool::Logging(myName.c_str(), "Processing finished. Connection closed.\n");
        }
        else
        {
            // ListenSSL 失败（例如 SSL_accept 失败）
            // 记录一个日志，然后循环继续，等待下一个连接
            tool::Logging(myName.c_str(), "Failed to establish a full connection. Waiting for the next one.\n");
            // 这里不需要 sleep，因为 accept() 本身就是阻塞的
        }
    }
#endif
